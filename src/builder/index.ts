...
export class SanIBuilder {
  ...
  constructor(options: SanIBuilderOptions) {
    ...
    if (options.staticDir) {
      const dirs = Array.isArray(options.staticDir)
        ? options.staticDir
        : [options.staticDir];
  ...
  async createPage(
    targetPath: string,
    sources: Array<StaticSourceInfo<any, any>>,
    options: {
      head?: HeadChildren;
    } = {}
  ) {
  ...
  async cacheSource<P, D>(
    source: Source<P, D>,
    params: P
  ): Promise<StaticSourceInfo<P, D>> {
  ...
  async cacheAsset(assetUrl: string) {
    ...
    try {
      const assetData = await (
        await axios.get(`http://localhost:3000/assets${encodeURI(pathname!)}`, {
  ...
  private buildJSContent(key: string, data: any) {
    ...
    return `0x${startIndex
      .toString(16)
      .padStart(8, "0")};${str}${JSON.stringify(data)}`;
  }
  ...
  private async writePage(page: PageStore, mapper: Record<string, string>) {
    ...
    if (head.metas) {
      ...
      for (const [name, content] of Object.entries(head.metas)) {
        const $meta = $metas
          .filter((_, meta) => $(meta).attr("name") === name)
          .first();
        if ($meta.length) {
    ...
    const mapperScript = `<script>window.__source_data_mapper=${JSON.stringify(
      mapper
    )}</script>`;
    const scripts = sources
      .map((source) => `<script src="${source.dataUri}"></script>`)
      .join("");
  ...
  private getSourceDataMapper() {
    return Array.from(this.sourceInfoCache.entries())
      .map(([key, info]) => [key, info.dataUri])
      .reduce<Record<string, string>>((mapper, [key, uri]) => {
        mapper[key] = uri;
        return mapper;
      }, {});
  }
