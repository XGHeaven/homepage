...
const assetHost = process.env.NODE_ENV !== 'production' ? 'http://localhost:3000' : ''
...
function transformToReactNode(node: Content | Content[], isTOC: boolean): ReactNode {
  ...
  switch(node.type) {
    case 'heading': {
      ...
      return React.createElement(tagName, {id: node?.data?.id}, transformToReactNode(node.children, isTOC))
    }
    case 'text': {
      return node.value
    }
    ...
    case 'list': {
      ...
      return React.createElement(listTag, {}, transformToReactNode(node.children, isTOC))
    }
    ...
    case 'link': {
      ...
      if (isTOC) {
        return <a href={node.url} target="_self">{child}</a>
      }
      return <a href={node.url} target="_blank">{child}</a>
    }
    ...
    case 'blockquote': {
      return <blockquote>{transformToReactNode(node.children, isTOC)}</blockquote>
    }
...
export const MarkdownRender = memo(function MarkdownRender(props: {
  ...
  return <Fragment>{transformToReactNode(props.node, props.isTOC ?? false)}</Fragment>
})
