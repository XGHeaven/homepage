...
export class ArticleResolver {
  ...
  @Query((of) => ArticlePagination)
  articles(@Args("limit") limit: number, @Args("offset") offset: number) {
    return this.articleService.getArticlesPagination(limit, offset);
  }
  ...
  async articleFieldIntro(
    @Parent() article: Article,
    @Args("maxLength", { nullable: true, defaultValue: 240, type: () => Int })
    maxLength: number,
    @Args("matchFragment", { nullable: true, defaultValue: true })
    matchFragment: boolean
  ) {
