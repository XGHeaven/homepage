...
export class CategoryService {
  ...
  async getAllCategory() {
    return (await Promise.all(Array.from(this.categoryMap.keys()).sort((a, b) => a.localeCompare(b)).map(name => this.getCategory(name))))
  }
  ...
  async getCategoryArticles(name: string) {
    ...
    return Promise.all(slugs.sort((a, b) => a.localeCompare(b)).map(slug => this.articleService.getArticle(slug)))
  }
}
