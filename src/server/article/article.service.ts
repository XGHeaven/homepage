...
export class ArticleService {
  ...
  private collection = new Collection<Article>((a, b) => a.date.getTime() - b.date.getTime())
  ...
  // Get Top 10
  getNewestArticles() {
    return this.getAllArticles().sort((a, b) => b.date.getTime() - a.date.getTime()).slice(0, 10)
  }
  ...
  async init(pattern: string) {
    ...
    await Promise.all(files.map(file => this.loadMarkdownFile(path.join(process.cwd(), file)).catch(e => {
      console.error(file)
  ...
  async loadMarkdownFile(mdPath: string) {
      ...
      .use(() => {
        return ((ast: Root, file) => {
          ...
          if (ast.children[0]?.type === 'yaml') {
            article.frontmatter = frontmatter = YAML.load(ast.children[0].value, {
              json: true
            })
          } else {
          ...
          const slug = article.frontmatter.slug || path.basename(mdPath, path.extname(mdPath))
          article.slug = slug

          const title = article.frontmatter.title || this.toPureString((toc.map?.children?.[0]?.children?.[0]?.children as any)?.[0]) || `${slug}(Untitled)`
          article.title = title
          ...
          if (frontmatter.coverImage) {
            ...
            if (coverImage.startsWith('http')) {
              ...
              const absolutePath = path.resolve(path.dirname(mdPath), slug, coverImage)
            ...
            switch (node.type) {
              case 'image': {
                ...
                if (!url.startsWith('http')) {
                  ...
                  const assetUri = assets.add(uri, path.join(path.dirname(mdPath), slug, url))
                  node.url = assetUri
                }
                break
              }
              ...
              case 'text': {
                ...
                if (text.startsWith('{%') && text.endsWith('%}')) {
                  // Hexo 标记符号处理
                  const [command, ...args] = text.slice(2, -2).trim().split(' ')
                  switch (command) {
                    case 'asset_img': {
                      const [imgPath] = args
                      const uri = path.join(`article/${slug}`, imgPath)
                      const imgUri = assets.add(uri, path.resolve(path.dirname(mdPath), slug, imgPath))
                      return {
                        type: 'image',
                        url: imgUri,
                        title: '',
    ...
    const { ast, toc, article } = file.data as { ast: Root, toc: mdtoc.TOCResult, article: Article }
    article.nodes = this.transformAST2JSON(filter(ast, { cascade: false }, (node: any): node is Node => { return node.type !== 'yaml' && node.type !== 'toml' }) as Node)?.children ?? []
    article.toc = toc.map
  ...
  private transformAST2JSON(node: Node) {
    ...
    if (newNode.children) {
      if (Array.isArray(newNode.children)) {
        newNode.children = newNode.children.map(child => this.transformAST2JSON(child))
      } else {
